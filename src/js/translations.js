

export const translations = {
    en: {
        nav: {
            home: "Home",
            written: "Written Books",
            translated: "Translated Books",
            contact: "Contact"
        },
        hero: {
            title: "Exploring the Fourth Way",
            subtitle: "A collection of written and translated works dedicated to spiritual development, self-remembering, and the teachings of G.I. Gurdjieff.",
            writtenBtn: "Browse Written Works",
            translatedBtn: "Explore Translations",
            callCaption: "The Call of Silence: Reflections on Awareness, Inspiration, and Intuition"
        },
        home: {
            aboutTitle: "About the Author & Translator",
            aboutText: "Dedicated to preserving and expanding the knowledge of the Fourth Way, these works aim to bring clarity to the profound spiritual teachings of the 20th century. Through careful translation and original writing, we strive to make these transformative ideas accessible to the modern seeker."
        },
        footer: {
            rights: "Fourth Way Books. All Rights Reserved.",
            madeWith: "Made in Canada with",
            by: "by",
            author: "Reza Naeim"
        },
        pages: {
            writtenTitle: "Original Works",
            writtenDesc: "Spiritual insights and commentaries on the Work.",
            translatedTitle: "Translations",
            translatedDesc: "Bringing essential spiritual texts to a new audience.",
            contactTitle: "Contact Us",
            contactDesc: "Get in touch for inquiries about books or translations.",
            readMore: "Read More"
        },
        contact: {
            name: "Name",
            email: "Email",
            message: "Message",
            send: "Send Message"
        },
        books: {
            callTitle: "The Call of Silence",
            callDesc: "Reflections on awareness, inspiration, and intuition. A book inviting the reader to the direct experience of silence and presence.",
            // innerPathTitle: "The Inner Path",
            // innerPathDesc: "A comprehensive guide to understanding self-observation and the principles of the Fourth Way in daily life.",
            // byAuthor: "By The Author",
            // originalBy: "Original by G.I. Gurdjieff",
            // awakeningTitle: "Awakening Consciousness",
            // awakeningDesc: "Exploring the mechanic of human behavior and the potential for spiritual awakening.",
            // practicalTitle: "Practical Work",
            // practicalDesc: "Specific exercises and practical applications of Gurdjieff's teachings for modern students.",
            // beelzebubTitle: "Beelzebub's Tales",
            // beelzebubDesc: "A fresh translation of Gurdjieff's magnum opus, aiming to clarify the objective critique of the life of man.",
            // meetingsTitle: "Meetings With Remarkable Men",
            // meetingsDesc: "The autobiography of Gurdjieff's search for truth, translated with attention to the cultural and spiritual context."
        },
        details: {
            callOfSilence: {
                title: "The Call of Silence",
                author: "By The Author",
                description: "The Call of Silence is a work centered on consciousness and self-discovery, inviting the reader to directly experience silence, presence, and inner observation. This work demonstrates that the deepest human insights emerge only when the mind grows quiet, allowing unmediated awareness to manifest. Along this path, the reader learns to observe the mind in the present moment and listen to inner guidance—a space where inspiration and intuition are recognized not as mystical concepts, but as natural aspects of daily life. Free from any specific school of thought or dogma, this book builds a bridge between the psychology of consciousness, the philosophy of presence, and the intuitive experience of the modern individual",
                image: "/assets/images/call-of-silence-cover.jpeg",
                link: "https://payhip.com/b/FY1c4",
                btnText: "Buy on Payhip"
            },
            awakening: {
                title: "Awakening Consciousness",
                author: "By The Author",
                description: "Exploring the mechanic of human behavior and the potential for spiritual awakening. This book delves into the practical aspects of self-study.",
                image: "https://via.placeholder.com/300x450/C5A059/ffffff?text=Awakening+Consciousness",
                link: "#",
                btnText: "Coming Soon"
            },
            practical: {
                title: "Practical Work",
                author: "By The Author",
                description: "Specific exercises and practical applications of Gurdjieff's teachings for modern students. A handbook for daily practice.",
                image: "https://via.placeholder.com/300x450/333333/ffffff?text=Practical+Work",
                link: "#",
                btnText: "Coming Soon"
            },
            inRealityOfBeing: {
                title: "In Search of Being",
                author: "Jeanne De Salzmann",
                translator: "Babak Lotfikish",
                description: "A definitive translation of Jeanne de Salzmann’s record of Gurdjieff’s wisdom, providing a clear and practical roadmap for those seeking to understand the objective reality of human life.",
                image: "/assets/images/in-search-of-being-cover.png",
                link: "#",
                btnText: "Coming Soon..."
            },
            meetings: {
                title: "Meetings With Remarkable Men",
                author: "Original by G.I. Gurdjieff",
                description: "The autobiography of Gurdjieff's search for truth, translated with attention to the cultural and spiritual context of his journeys.",
                image: "https://via.placeholder.com/300x450/444444/ffffff?text=Meetings+With+Rm",
                link: "#",
                btnText: "Read Online"
            }
        }
    },
    fa: {
        nav: {
            home: "خانه",
            written: "کتاب‌های تالیفی",
            translated: "کتاب‌های ترجمه شده",
            contact: "تماس با ما"
        },
        hero: {
            title: "کاوش در راه چهارم",
            subtitle: "مجموعه‌ای از آثار تالیفی و ترجمه شده با تمرکز بر رشد معنوی، خودیادآوری و آموزه‌های جی.آی. گورجیف.",
            writtenBtn: "مشاهده آثار تالیفی",
            translatedBtn: "کاوش در ترجمه‌ها",
            callCaption: "ندای سکوت: تاملاتی درباره آگاهی، الهام و شهود"
        },
        home: {
            aboutTitle: "درباره نویسنده و مترجم",
            aboutText: "با هدف حفظ و گسترش دانش راه چهارم، این آثار سعی در شفاف‌سازی آموزه‌های معنوی عمیق قرن بیستم دارند. از طریق ترجمه دقیق و تالیف اصیل، ما می‌کوشیم تا این ایده‌های تحول‌آفرین را برای جویندگان امروزی قابل دسترس کنیم."
        },
        footer: {
            rights: "تمامی حقوق محفوظ است.",
            madeWith: "ساخته شده در کانادا با",
            by: "توسط",
            author: "رضا نعیم"
        },
        pages: {
            writtenTitle: "آثار تالیفی",
            writtenDesc: "بینش‌های معنوی و تفاسیر بر کار.",
            translatedTitle: "ترجمه‌ها",
            translatedDesc: "ارائه متون معنوی ضروری به مخاطبان جدید.",
            contactTitle: "تماس با ما",
            contactDesc: "برای سوالات در مورد کتاب‌ها یا ترجمه‌ها با ما در تماس باشید.",
            readMore: "بیشتر بخوانید"
        },
        contact: {
            name: "نام",
            email: "ایمیل",
            message: "پیام",
            send: "ارسال پیام"
        },
        books: {
            callTitle: "ندای سکوت",
            callDesc: "تاملاتی درباره آگاهی، الهام و شهود. دعوت خواننده به تجربه مستقیم سکوت، حضور و مشاهده درونی.",
            innerPathTitle: "راه درون",
            innerPathDesc: "راهنمای جامع برای درک مشاهده خود و اصول راه چهارم در زندگی روزمره.",
            byAuthor: "نوشته نویسنده",
            originalBy: "اثر جی.آی. گورجیف",
            awakeningTitle: "بیدار کردن آگاهی",
            awakeningDesc: "کاوش در مکانیسم رفتار انسان و پتانسیل بیداری معنوی.",
            practicalTitle: "کار عملی",
            practicalDesc: "تمرینات خاص و کاربردهای عملی آموزه‌های گورجیف برای شاگردان امروزی.",
            beelzebubTitle: "قصه‌های بلزباب",
            beelzebubDesc: "ترجمه‌ای تازه از شاهکار گورجیف، با هدف شفاف‌سازی نقد عینی زندگی بشر.",
            meetingsTitle: "ملاقات با مردان برجسته",
            meetingsDesc: "زندگینامه جستجوی حقیقت گورجیف، ترجمه شده با توجه به زمینه فرهنگی و معنوی."
        },
        details: {
            callOfSilence: {
                title: "ندای سکوت",
                author: "نوشته نویسنده",
                description: "«.ندای سکوت» کتابی در حوزهٔ آگاهی و خودشناسی است که خواننده را به تجربهٔ مستقیم سکوت، حضور و مشاهدهٔ درونی دعوت می‌کند.این اثر نشان می‌دهد که عمیق‌ترین دریافت‌های انسان زمانی پدیدار می‌شوند که ذهن آرام می‌گیرد و آگاهیِ بی‌واسطه مجال ظهور می‌یابد. خواننده در این مسیر می‌آموزد ذهن را در لحظهٔ اکنون مشاهده کند و به راهنمایی درونی گوش بسپارد؛ جایی که الهام و شهود نه به‌عنوان مفاهیمی رازآلود، بلکه به‌عنوان بخشی طبیعی از زندگی روزمره شناخته می‌شوند.این کتاب، بدون وابستگی به مکتب یا آیینی خاص، پلی میان روان‌شناسی آگاهی، فلسفهٔ حضور و تجربهٔ شهودی انسان معاصر می‌سازد",
                image: "/assets/images/call-of-silence-cover.jpeg",
                link: "https://payhip.com/b/FY1c4",
                btnText: "خرید در Payhip"
            },
            awakening: {
                title: "بیدار کردن آگاهی",
                author: "نوشته نویسنده",
                description: "کاوش در مکانیسم رفتار انسان و پتانسیل بیداری معنوی. این کتاب به جنبه‌های عملی خودشناسی می‌پردازد.",
                image: "https://via.placeholder.com/300x450/C5A059/ffffff?text=Awakening+Consciousness",
                link: "#",
                btnText: "به زودی"
            },
            practical: {
                title: "کار عملی",
                author: "نوشته نویسنده",
                description: "تمرینات خاص و کاربردهای عملی آموزه‌های گورجیف برای شاگردان امروزی. راهنمایی برای تمرین روزانه.",
                image: "https://via.placeholder.com/300x450/333333/ffffff?text=Practical+Work",
                link: "#",
                btnText: "به زودی"
            },
            inRealityOfBeing: {
                title: "واقعیت بودن",
                author: "اثر جین دی سالزمن",
                translator: "بابک لطفیکیش",
                description: "ترجمه‌ای نهایی از مکتوبات ژان دو سالزمن درباره حکمت گورجیف؛ نقشه‌راهی روشن و عملی برای آنانی که در جستجوی درک واقعیت عینی حیات بشری هستند.",
                image: "/assets/images/in-search-of-being-cover.png",
                link: "#",
                btnText: "...به زودی"
            },
            meetings: {
                title: "ملاقات با مردان برجسته",
                author: "اثر جی.آی. گورجیف",
                description: "زندگینامه جستجوی حقیقت گورجیف، که با توجه به زمینه فرهنگی و معنوی سفرهای او ترجمه شده است.",
                image: "https://via.placeholder.com/300x450/444444/ffffff?text=Meetings+With+Rm",
                link: "#",
                btnText: "مطالعه آنلاین"
            }
        }
    }
};
